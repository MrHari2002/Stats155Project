...
Which stat is most impacted by whether the game is home or away?
...
gamedataWithOutTeam <- game_logs (seasons = 2022)
...
This plot is a logistic model of the relationship between the number of points a player has and the odds that their team wins. The initial point is 0.5, suggesting there is an equal probability that a player's team will win or lose when they have zero points, but every additional point increases the likelihood of the player's team win.

```{r}
ggplot(gamedata, aes(x = pts, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not")+xlab("Pts scored by a indiviual player in the game")
...
ggplot(gamedata, aes(x = pctFG, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not") + xlab("pctFG by a indiviual player")
...
ggplot(gamedata, aes(x = tov, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not")+xlab("tov by an indiviual player")
...
ggplot(gamedata, aes(x = team_pctFG3, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not")+xlab("team_pctFG3")
...
ggplot(gamedata, aes(x = team_ast, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not")+xlab("Total Team Assists")
...
ggplot(gamedata, aes(x = team_stl, y = as.numeric(isWin))) +
  geom_jitter() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, fullrange = TRUE) +
  ylab("Is Win Or Not")+xlab("Total Team Steals")
...
\
```{r}
ggplot(gamedata, aes(y = team_pts,x = locationGame)) +
  geom_boxplot()

ggplot(gamedata, aes(y = team_ast,x = locationGame)) +
  geom_boxplot()

ggplot(gamedata, aes(y = team_stl,x = locationGame)) +
  geom_boxplot()

ggplot(gamedata, aes(y = team_reb,x = locationGame)) +
  geom_boxplot()

ggplot(gamedata, aes(y = team_blk,x = locationGame)) +
  geom_boxplot()
...
```{r}
# Building a team stats that has only team stats
team_stats <- 
  gamedata %>%
  select(nameTeam,idGame,locationGame, team_pts,team_ast,team_blk,team_reb,team_stl,isWin) %>%
  arrange(idGame) %>%
  unique()
  
  
basketball_model_1 <- 
  glm(isWin ~ team_pts + team_ast + team_blk + team_reb + team_stl, data = team_stats)
...
exp(0.018114)
```
Each the interpreation below is interpreted controlling for other predicators in the model

team_pts -> The odds of a team winning increases by 1.6641 percent for every point that the team scores in the game.

team_ast -> The odds of a team winning increases by 0.5624 percent for every assist that the team achieves in the game.

team_blk -> The odds of a team winning increases by 1.1311 percent for every block that the team has during the game.

team_reb -> The odds of a team winning increases by 2.0051 percent for every rebound that the team has during the game.

team_stl -> The odds of a team winning increases by 1.8279 percent for every steal that the team has during the game.
...
basketball_model_2 <- 
  lm(team_pts ~ locationGame,data = gamedata)
...
pts -> The odds of a team winning increases by 0.1804 percents for every point that a player scores in the game.

ast -> The odds of a team winning increases by 0.6364 percents for every assist that a player achieves in the game.

blk -> The odds of a team winning increases by 1.3184 percents for every block that a player has during the game.

treb -> The odds of a team winning increases by 0.1978 percents for every rebound that a player has during the game.

stl -> The odds of a team winning increases by 0.6126 percents for every steal that a player has during the game.
...
TeamData <- gamedata %>% 
  select(nameTeam, idGame, locationGame, isWin, slugMatchup, team_pts) %>%
  arrange(idGame) %>%
  unique()

TeamData %>% 
  ungroup() %>%
  filter(locationGame == "H",isWin==TRUE) %>% 
  count()
...
#These were commented out considering that 
#filter(gamedataWithOutTeam,locationGame=="H",isWin==TRUE) %>% summarize(n())
#filter(gamedataWithOutTeam,locationGame=="H") %>% summarize(n())
#7203/13067
#filter(gamedataWithOutTeam,locationGame=="A",isWin==TRUE) %>% summarize(n())
#5917/12972
#filter(gamedataWithOutTeam,locationGame=="A") %>% summarize(n())


```

locationGameH -> The odds of a team winning a game is 0.099 higher for games that are played at home than when a game is away. Hometeams has 52% chances of winning in total, so according to this model home-court advantage is significant.
